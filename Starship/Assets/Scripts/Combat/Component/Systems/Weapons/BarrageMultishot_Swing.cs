...
{
    public class BarrageMultishot_Swing : SystemBase, IWeapon
    {
        public BarrageMultishot_Swing(IWeaponPlatform platform, WeaponStats weaponStats, Factory.IBulletFactory bulletFactory, int keyBinding, IShip ship)
            : base(keyBinding, weaponStats.ControlButtonIcon, ship)
        {
            ...
            if (_spread == -999f)
            {
                ...
                {
                    bulletItems[i].num = i;
                    bulletItems[i].spread = -_maxspread / 2 + i * _maxspread / _magazine;
