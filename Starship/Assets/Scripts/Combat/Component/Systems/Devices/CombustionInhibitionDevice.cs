...
{
    public class CombustionInhibitionDevice : SystemBase, IDevice, IFeaturesModification, IStatsModification
    {
        public CombustionInhibitionDevice(IShip ship, DeviceStats deviceSpec, int keyBinding)
        ...
        public bool TryApplyModification(ref Resistance data)
        {
            if (_isEnabled)
            {
                //data.Energy = 0.8f + data.Energy*0.2f;
                data.Heat = 0.8f + data.Heat * 0.2f;
                //data.Kinetic = 0.8f + data.Kinetic * 0.2f;
            }
