...
namespace Session.Content
{
	public class RegionData : ISerializableData
	{
		...
		private void Deserialize(byte[] buffer, int gameSeed)
		{
			...
			if (version != CurrentVersion && !TryUpgrade(ref buffer, version, gameSeed, _regionMap))
			{
		...
		private static bool TryUpgrade(ref byte[] data, int version, int gameSeed, RegionMap regionMap)
		{
			if (version == 1)
			{
			...
			if (version == 2)
			{
				data = Upgrade_2_3(data, gameSeed, regionMap).ToArray();
				version = 3;
			}
		...
		private static IEnumerable<byte> Upgrade_2_3(byte[] buffer, int gameSeed, RegionMap regionMap)
		{
			var index = 0;
			...
			var regionFactions = defeatedFleetCount.Keys.ToDictionary(regionId => regionId, regionId =>
			{
				...
				//StarLayout.IdToPosition(regionId, out var x, out var y);
				//regionMap.PublicGetRsndomSize(x, y, out var size);

				//var homeStar = RegionLayout.GetRandomSizeRegionHomeStar(regionId,size);
