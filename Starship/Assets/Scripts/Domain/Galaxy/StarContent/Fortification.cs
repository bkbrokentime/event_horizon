...
namespace Galaxy.StarContent
{
    public class Fortification
    {
        ...
        {
            return Fleet.Fortification(_starData.GetRegion(starId), _database);
        }
        ...
        {
            public Facade(Fortification fortification, int starId)
            {
                _fortification = fortification;
                _starId = starId;
            }

            public Model.Military.IFleet CreateFleet() { return _fortification.CreateFleet(_starId); }
            public bool IsDefeated { get { return _fortification.IsDefeated(_starId); } }
            public void Attack() { _fortification.Attack(_starId); }

            private readonly Fortification _fortification;
