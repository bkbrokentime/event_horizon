...
namespace Constructor.Ships.Modification
{
    public class LayoutModifications
    {
        ...
        public LayoutModifications(Ship ship)
        {
            ...
            Second_size = ship.SecondLayout.Size;
            _layout = new char[_size*_size];
            Second_layout = new char[Second_size * Second_size];
            Initialize();
        }
        ...
        public Layout BuildSecondLayout()
        {
            return new Layout(new string(Second_layout));
        }
        ...
        private void Initialize()
        {
            ...
            if (_ship.SecondLayout.Data != string.Empty)
            {
                layout = _ship.SecondLayout;
                size = layout.Size;

                Assert.AreEqual(size * size, Second_layout.Length);
                ...
                for (var i = 0; i < size; ++i)
                {
                    for (var j = 0; j < size; ++j)
                    {
                        ...
                        if (cellType != (char)CellType.Empty)
                        {
                            Second_layout[i * size + j] = cellType;
                        }
                        else if (layout[x, y - 1] != (char)CellType.Empty || layout[x - 1, y] != (char)CellType.Empty ||
                                 layout[x + 1, y] != (char)CellType.Empty || layout[x, y + 1] != (char)CellType.Empty)
                        {
                            Second_layout[i * size + j] = (char)CellType.Custom;
                        }
                        else
                        {
                            Second_layout[i * size + j] = (char)CellType.Empty;
                        }
                    }
                }
            }
        ...
        private readonly int Second_size;
        private readonly char[] _layout;
        private readonly char[] Second_layout;
